/**
 * Factory Droid local filesystem importer.
 *
 * Factory Droid stores session data in ~/.factory or ~/.factory/sessions.
 * Expected structure: JSON files or JSONL with session entries.
 *
...
export class FactoryDroidImporter implements ToolImporter {
  ...
  canHandle(sourcePath: string): boolean {
    try {
      ...
      if (stat.isFile()) {
        return this.isDroidFile(sourcePath);
      }
      const dirName = basename(sourcePath).toLowerCase();
      if (dirName === '.factory' || dirName === 'factory' || dirName === 'factory-droid') return true;
      if (dirName === 'sessions') {
        const parent = basename(join(sourcePath, '..')).toLowerCase();
        if (parent === '.factory' || parent === 'factory') return true;
      }
      return this.hasDroidFiles(sourcePath);
    } catch {
      return false;
    }
  }
  ...
  parse(options: ImportOptions): ImportResult {
    ...
    try {
      ...
      result.errorDetails.push(`Factory Droid parse error: ${sanitizeForOutput(msg)}`);
    }
  ...
  private isDroidFile(filePath: string): boolean {
    const ext = extname(filePath).toLowerCase();
    if (ext !== '.json' && ext !== '.jsonl') return false;
    const name = basename(filePath).toLowerCase();
    return name.includes('droid') || name.includes('factory') || name.includes('mission');
  }

  private hasDroidFiles(dirPath: string): boolean {
    try {
      const entries = readdirSync(dirPath);
      return entries.some((e) => this.isDroidFile(join(dirPath, e)));
    } catch {
      return false;
    }
  }
  ...
  private extractSummary(messages: Record<string, unknown>[]): string | undefined {
    ...
    return `${messages.length} messages in Factory Droid session`;
  }
}
